...
| **Multi-tenancy** | All queries scoped by `organization_id`; tested for cross-org isolation |
| **Caching** | In-memory report cache keyed by `(org_id, asset_type)` to avoid redundant LLM calls |
...
An `AgentExecutor` with access to all four chains as LangChain tools. The agent autonomously decides which tools to invoke — it can chain queries, look up asset IDs, then run risk analysis, all from a single prompt.
...
curl -X POST http://localhost:8000/agent \
  ...
  -d '{
    "organization_id": "org_default",
    "prompt": "Find all certificates and run a risk analysis on them"
  }'
```

The agent will:
1. Call `query_assets("certificates")` → get asset IDs
2. Call `analyze_risk([...ids...])` → produce risk assessment
3. Return a synthesized answer

### Report Caching

Report generation results are cached in-memory by `(org_id, asset_type)`. The response includes `"cached": true/false` so clients know whether the result was freshly generated. This avoids redundant LLM calls for repeated requests.

### Multi-Tenancy

All database queries are scoped by `organization_id`. Assets from `org_A` are never visible to `org_B`. This is enforced at the CRUD layer and tested explicitly (see `TestMultiTenancy` in `tests/test_import.py`).
...
│       ├── agent.py       # AgentExecutor with tool-use (bonus)
